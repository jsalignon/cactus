...
# cactus revisions ran: 57a2e7f516d9ac7114f389c0f226ca01ae63db7a
...
--species $species -profile singularity -r $cactus_version $latest_flag -resume --references_dir $references_dir --executor_local_cpus $cpu_nb --executor_local_memory $memory_size --heatmaps_filter__CHIP "$heatmaps_filters_chip" --heatmaps_filter__motifs "$heatmaps_filters_motifs" --common_padj_breaks "$padj_breaks" --common__heatmaps_ggplot "c( 12, 12, 12 )" 
...
## repeating the peak-self analysis with a lower FDR cutoff
species=human ; cd $case_study_dir/$species
