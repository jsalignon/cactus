...
The parameter *params.references_dir* set in the *./.cactus.config* file should be set to indicate in which directory the references should be downlaoded. In addition, the parameter *threads* can be set to determine the number of thread used by pigz for uncompressing the references archive files. 
