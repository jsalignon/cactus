...
### Parameters
- **_params.nb_threads_bowtie2_**: number of threads used by Bowtie2.
...
- **_params.nb_threads_deeptools_**: number of threads used by DeepTools.
...
### Parameters
- **_params.nb_threads_botwie2_**: number of threads used by Bowtie2.

 
## ATAC_QC_reads__gathering_all_stat

### Description
This process gather all reads statistics from previous processes (starting with *ATAC_QC_reads__*) for a given sample, computes a few more stastics and creates a summary file.  
...
### Description
A [MultiQC](https://pubmed.ncbi.nlm.nih.gov/27312411/) html report is made that aggregates all basic FastQC quality controls, and the custom statistics files generated by cactus.

### Outputs
- **MultiQC report** (.html files) in `Figures_Individual/1_Preprocessing` and `Figures_Merged/1_Preprocessing`
